...
def record_payment():
    ...
    if user:
        ...
        # Ensure system product for payments exists
        pay_prod = Products.query.get('PAYMENT')
        if not pay_prod:
            pay_prod = Products(upc_code='PAYMENT', description='Account Payment', price=0, stock_level=0, category='System')
            db.session.add(pay_prod)
        # Log as negative amount (credit)
...
def delete_user(user_id):
    if int(session.get('user_id')) == user_id: return redirect(url_for('main.manage_users'))
...
def monthly_report():
    ...
    tx_by_user = {}
    for t, p in tx_rows:
        tx_by_user.setdefault(t.user_id, []).append({"when": t.transaction_date.strftime("%Y-%m-%d"), "desc": (p.description if p else "Item"), "amount": float(t.amount or 0)})
...
def nuke_transactions():
    Transactions.query.delete(); db.session.commit()
    flash("HISTORY NUKED.", "danger")
    return redirect(url_for('main.index', open_admin=1))

@main.route('/admin/reset-balances')
def reset_balances():
    Users.query.update({Users.balance: 0.00}); db.session.commit()
