...
def process_barcode(barcode):
    ...
    # Prevent purchasing when stock is zero (UI might still be bypassed via direct URL/scan)
    if product and getattr(product, 'stock_level', None) is not None and product.stock_level <= 0:
...
def reset_balances():
    ...
    if admin and admin.is_admin:
        ...
        db.session.commit()
        flash("All account balances reset to 
def process_barcode(barcode):
    barcode = barcode.strip()
    user = Users.query.filter_by(card_id=barcode).first()
    if user:
...
def logout():
    ...
    return redirect(url_for('main.index')).00.", "warning")
    return redirect(url_for('main.index', open_admin=1))
