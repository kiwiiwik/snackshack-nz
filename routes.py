...
def get_product(barcode):
    ...
    if p:
        return jsonify({"found": True, "mfg": p.manufacturer, "desc": p.description, "size": p.size, "price": str(p.price), "cat": p.category, "soh": p.stock_level})
    try:
...
def save_product_manual():
    ...
    # Corrected assignments for Brand (Manufacturer) and Size
    p.manufacturer = request.form.get('manufacturer')
    p.size = request.form.get('size')
    p.description = request.form.get('description')
    p.price = Decimal(request.form.get('price', '0.00'))
...
def select_user(user_id):
    ...
    u.last_seen = datetime.utcnow(); db.session.commit()
...
def pin_set():
    if 'user_id' in session:
        ...
        if u and pin.isdigit(): u.pin = pin; db.session.commit(); flash("PIN enabled.", "success")
    return redirect(url_for('main.index'))
